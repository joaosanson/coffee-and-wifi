...
class CafeForm(FlaskForm):
	...
	closing_time = StringField(label='Closing Time e.g. 5:30AM', validators=[DataRequired()])
...
def add_cafe():
	...
	if form.validate_on_submit():
		...
		f.write(f"\n{form.cafe.data}, {form.location_url.data}, {form.open_time.data}, {form.closing_time.data},"
...
def cafes():
	with open('cafe-data.csv', newline='', encoding="utf8") as csv_file:
		...
		list_of_rows_without_head = []
		for row in list_of_rows[1:]:
			list_of_rows_without_head.append(row)

	return render_template('cafes.html', cafes=list_of_rows, cafes_x=list_of_rows_without_head)
